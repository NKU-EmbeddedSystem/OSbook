...
if [ ! -e "output/lab3_result.txt" ]; then
    echo "输出文件不存在，lab4 多线程部分最终分数为: 0。"
    exit 1
elif [ ! -e "output/lab4_thread_result.txt" ]; then
    echo "输出文件不存在，lab4 多线程部分最终分数为: 0。"
    exit 1
fi
...
file2="output/lab4_thread_result.txt"
...
# 比较MD5值是否相等
if [[ "$md5_1" == "$md5_2" ]]; then
    echo "文件1和文件2的排序结果的MD5值相等, lab4 多线程部分最终分数为: 100"
else
    echo "文件1和文件2的排序结果的MD5值不相等,lab4 多线程部分最终分数为: 0"
fi
